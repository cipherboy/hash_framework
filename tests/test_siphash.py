import cmsh
from hash_framework.algorithms._siphash import *
...
def test_known_value():
    ...
    block = []
    assert int(siphash(model, key, block)) == 0x1e924b9d737700d7

    key = model.to_vec(0x000102030405060708090a0b0c0d0e0f, 128)
    block = model.to_vec(0x000102030405060708090a0b0c0d0e, 120)
    assert int(siphash(model, key, block)) == 0xa129ca6149be45e5

    key = model.to_vec(0x00, 128)
    block = model.to_vec(0x48656c6c6f20776f726c64, 88)
    assert int(siphash(model, key, block)) == 0xc9e8a3021f3822d9

    key = model.to_vec(0x00, 128)
    block = model.to_vec(0x3132333435363738313233, 88)
    assert int(siphash(model, key, block)) == 0xf95d77ccdb0649f
