...
def from_tags(db):
    r = 32
    tags = db.execute(
        "SELECT DISTINCT tag FROM c_md5 WHERE tag LIKE '%r" + str(r + 4) + "%';"
    ).fetchall()
    print(len(tags))
    return list(
        map(
...
def tag_neighbors(db):
    r = 44
    q = (
        "SELECT DISTINCT tag FROM c_md5 WHERE tag LIKE '%r"
        + str(r - 4)
        + "%' AND ROWID > 91829;"
    )
    print(q)
...
def tag_fill_in(db):
    r = 32
    tags = db.execute(
        "SELECT DISTINCT tag FROM c_md5 WHERE tag LIKE '%r" + str(r) + "%';"
    ).fetchall()
    print(len(tags))
    lt = list(
        map(
