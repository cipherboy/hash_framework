...
def tag_neighbors(db):
    r = 44
    q = (
        "SELECT DISTINCT tag FROM c_md4 WHERE tag LIKE '%r"
        + str(r - 4)
        + "%' AND ROWID > 91829;"
    )
    print(q)
...
if __name__ == "__main__":
    ...
    work = kernel.gen_work(
        48, [dobbertins_base, wangs_base, schlaffers_base, kasselman_base, sasakis_base]
    )
    ...
    work_mapped = list(
        map(
            lambda y: kernel.work_to_args(
                "md4", y, start_state=start_state, start_block=start_block
            ),
            work,
        )
    )
