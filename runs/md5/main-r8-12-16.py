...
def from_tags(db):
    r = 32
    tags = db.execute("SELECT DISTINCT tag FROM c_md5 WHERE tag LIKE '%r" + str(r + 4) + "%';").fetchall()
    print(len(tags))
    return list(map(lambda x: (r, tuple(map(int, x[9:].split('-')))), map(lambda y: ''.join(y), tags)))

def tag_neighbors(db):
    r = 44
    q = "SELECT DISTINCT tag FROM c_md5 WHERE tag LIKE '%r" + str(r - 4) + "%' AND ROWID > 91829;"
    print(q)
...
def tag_fill_in(db):
    r = 32
    tags = db.execute("SELECT DISTINCT tag FROM c_md5 WHERE tag LIKE '%r" + str(r) + "%';").fetchall()
    print(len(tags))
    lt = list(map(lambda x: tuple(map(int, x[9:].split('-'))), map(lambda y: ''.join(y), tags)))
    ne = set()
    for delta in lt:
...
def tag_neighborhood_extend(db, kernel):
    ...
    # 92411
    q = "SELECT DISTINCT " + ','.join(c) + " FROM c_md5 WHERE tag LIKE '%r" + str(r) + "%' AND ROWID > 91811;"
    # q = "SELECT DISTINCT " + ','.join(c) + " FROM c_md5 WHERE tag LIKE '%r" + str(r) + "%' AND ROWID = 91811;"
    print(q)
...
if __name__ == "__main__":
    ...
    hash_framework.attacks.collisions.create_db(algo, db)
    assert(False)
