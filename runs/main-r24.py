...
def from_tags(db):
    r = 32
    tags = db.execute("SELECT DISTINCT tag FROM c_md4 WHERE tag LIKE '%r" + str(r + 4) + "%';").fetchall()
    print(len(tags))
    ...
    for delta in lt:
        for n in range(1, 2):
            for e in hash_framework.attacks.second.generate_test_delta(delta, n, r):
                if e not in lts and (16 in e or 17 in e or 18 in e or 19 in e):
                    ne.add(e)
    print(len(lt))
    print(len(ne))
...
if __name__ == "__main__":
    ...
    compute += _h("10.1.30.6", 4)
    compute += _h("10.1.30.7", 4)
    compute += _h("10.1.30.8", 8)
    compute += _h("10.1.30.9", 8)
