...
def __main__():
    ...
    while row != None:
        ...
        if w in results:
            results[w][input_error][output_margin] = run_result
        else:
            print((w, input_error, output_margin))

        row = cur.fetchone()

    for w in results:
        draw_table(results[w], 25*w, "/tmp/table-od-margin-" + sys.argv[2] + "-w" + str(w) + "-o0.png")
