...
def main():
    ...
    for i in range(0, num_points):
        for j in range(i+1, num_points):
            k_i = _siphash.siphash(m, known_128, values_64[i], outlen=8, cROUNDS=cr, dROUNDS=dr)
            k_j = _siphash.siphash(m, known_128, values_64[j], outlen=8, cROUNDS=cr, dROUNDS=dr)

            g_i = _siphash.siphash(m, guess, values_64[i], outlen=8, cROUNDS=cr, dROUNDS=dr)
            g_j = _siphash.siphash(m, guess, values_64[j], outlen=8, cROUNDS=cr, dROUNDS=dr)
