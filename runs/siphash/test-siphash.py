...
def bit64_to_hex(num):
    t1 = num[0:32]
    t2 = num[32:]
    return hf.boolean.b_block_to_hex_reverse([hf.boolean.b_tonum(t1), hf.boolean.b_tonum(t2)])
...
def run():
    ...
    for nb in [32, 48, 64]:
        ob = 16
        kub = 64
        print((ob, kub, nb, br, fr))
        mdir = "ob" + str(ob) + "-kub" + str(kub) + "-nb" + str(nb) + "-br" + str(br) + "-fr" + str(fr)
        m = hf.models()
        m.start(mdir, False)
        base_dir = m.model_dir + "/" + mdir
        _sh.build_test_key_leakage(dir=base_dir, key_unknown_bits=kub, number_of_blocks=nb, block_rounds=br, final_rounds=fr, output_bits=ob)
        m.collapse()
        m.build()
        m.remote = False
        m.run(count=2)
