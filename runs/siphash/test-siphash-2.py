        ...
        for br in [1, 2]:
            for nb in range(1, 17):
                for kub in range(0, 65, 2):
                    print((kub, nb, br, fr))
                    mdir = "kub" + str(kub) + "-nb" + str(nb) + "-br" + str(br) + "-fr" + str(fr)
                    m = hf.models()
                    m.start(mdir, False)
                    base_dir = m.model_dir + "/" + mdir
                    _sh.build_test_key_leakage(dir=base_dir, key_unknown_bits=kub, number_of_blocks=nb, block_rounds=br, final_rounds=fr)
                    m.collapse()
                    m.build()
                    m.remote = False
                    m.run(count=2)
