...
def md4(model, block, iv=[0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476], rounds=48):
    ...
    for round_index in range(0, 16):
        if round_index >= rounds:
            break
        state = md4_round(block, md4r1, state, xchoice[round_index % 16], shifts[round_index % 4])
    ...
    for round_index in range(16, 32):
        if round_index >= rounds:
            break
        state = md4_round(block, md4r2, state, xchoice[round_index % 16], shifts[round_index % 4])
        result_rounds.append(state[1])
    ...
    for round_index in range(32, 48):
        if round_index >= rounds:
            break
        state = md4_round(block, md4r3, state, xchoice[round_index % 16], shifts[round_index % 4])
        result_rounds.append(state[1])
