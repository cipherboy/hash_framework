...
class SHA3Differences(Kernel):
    name = "sha3differences"
    ...
    def gen_work(rounds, w, input_errors, output_errors):
        # Function Margins
    ...
    def build_tag(self):
        tag = str(self.jid) + self.build_cache_tag() + "-ie" + str(self.input_error)
        tag += "-oe" + str(self.output_error)
        return tag

    def build_cache_tag(self):
        base = "s3d-" + self.algo_name + "-r" + self.srounds + "-w" + str(self.w)
        return base
