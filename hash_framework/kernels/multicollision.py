...
class Multicollision(Kernel):
    ...
    def work_to_args(algo_name, work, start_state=None, start_block=None):
        rounds, base = work
        d = {"algo": algo_name, "rounds": rounds, "cms_args": [], "base": base}
    ...
    def pre_run(self):
        ...
        if not os.path.exists(cache_path):
            ...
            if self.create_cache_dir(cache_dir_path):
                ...
                models.generate(
                    self.algo, ["h1", "h2", "h3", "h4"], rounds=self.rounds, bypass=True
                )
                attacks.collision.write_constraints(
                    self.algo,
                    prefixes=["h1", "h2"],
                    name="98-h1-h2-constraints.txt",
                    out_name="ccollisionh1h2",
                )
                attacks.collision.write_constraints(
                    self.algo,
                    prefixes=["h3", "h4"],
                    name="98-h3-h4-constraints.txt",
                    out_name="ccollisionh3h4",
                )
                attacks.collision.write_optional_differential(self.algo)
                attacks.collision.write_same_state(
                    self.algo,
                    prefixes=["h1", "h2"],
                    name="01-h1-h2-state.txt",
                    out_name="cstateh1h2",
                )
                attacks.collision.write_same_state(
                    self.algo,
                    prefixes=["h3", "h4"],
                    name="01-h3-h4-state.txt",
                    out_name="cstateh3h4",
                )
                attacks.collision.write_same_blocks(
                    self.algo,
                    prefixes=["h1", "h3"],
                    name="08-h1-h3-block.txt",
                    out_name="cblocksh1h3",
                )
                attacks.collision.write_same_blocks(
                    self.algo,
                    prefixes=["h2", "h4"],
                    name="08-h2-h4-block.txt",
                    out_name="cblocksh2h4",
                )
        ...
        attacks.collision.connected.loose.distributed_new_neighbor(
            ...
            [],
            [],
            base_path + "/07-h1-h2-differential.txt",
            ["h1", "h2"],
            "cdifferentialsh1h2",
        )
        attacks.collision.connected.loose.distributed_new_neighbor(
            ...
            [],
            [],
            base_path + "/07-h3-h4-differential.txt",
            ["h3", "h4"],
            "cdifferentialsh3h4",
        )
