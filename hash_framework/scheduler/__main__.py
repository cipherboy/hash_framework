...
success = {"status": "success", "status_code": 200, "message": "OK"}
        ...
        for task in next_task_obj["ordered_tasks"]:
            if "remaining_jobs_delta" in task and "current_threads_delta" in task:
                db.prepared(
                    q,
                    [
                        task["remaining_jobs_delta"],
                        task["current_threads_delta"],
                        task["id"],
                    ],
                    commit=True,
                )
...
def pop_tasks(db, host_id, limit=1):
    ...
    for task in next_task_obj["ordered_tasks"]:
        remaining_threads = (
            task["current_threads"] < task["max_threads"] or task["max_threads"] == -1
        )
