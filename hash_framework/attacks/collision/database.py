...
def __insert__(db, table, values, commit=False, rowid=True):
    ...
    assert type(values) == dict or (
        type(values) == list and len(values) > 0 and type(values[0]) == dict
    )
    rids = []
    if type(values) == dict:
        values = [values]
...
def verify_collision(algo, col):
    ...
    for i in range(0, algo.state_size // algo.int_size):
        if (
            jet["h1o" + str(i)] != jet["h2o" + str(i)]
            or jet["h1o" + str(i)] != col["h1o" + str(i)]
            or jet["h2o" + str(i)] != col["h2o" + str(i)]
        ):
            return False

    for i in range(0, algo.rounds):
        if (
            jet["h1i" + str(i)] != col["h1i" + str(i)]
            or jet["h2i" + str(i)] != col["h2i" + str(i)]
        ):
            return False
