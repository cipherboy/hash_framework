...
def write_constraints(
    algo, prefixes=["h1", "h2"], name="98-collision.txt", out_name="ccollision"
):
    assert (
        type(prefixes) == list
        and len(prefixes) == 2
        and type(prefixes[0]) == str
        and type(prefixes[1]) == str
    )
    c = ["and"]
...
def write_optional_differential(
    algo, prefixes=["h1", "h2"], name="06-blocks.txt", out_name="cblocks"
):
    assert (
        type(prefixes) == list
        and len(prefixes) == 2
...
def write_same_state(
    algo, prefixes=["h1", "h2"], name="01-state.txt", out_name="cstate"
):
    assert (
        type(prefixes) == list
        and len(prefixes) == 2
...
def write_same_blocks(
    algo, prefixes=["h1", "h2"], name="08-blocks.txt", out_name="cblock"
):
    assert (
        type(prefixes) == list
        and len(prefixes) == 2
