...
class database:
    ...
    def populate(algo, db, start, target):
        hash_framework.attacks.collision.connected.loose.database.add(
            algo, db, start, "wq"
        )
        hash_framework.attacks.collision.connected.loose.database.add(
            algo, db, start, "r"
        ...
        hash_framework.attacks.collision.connected.loose.database.add(
            algo, db, target, "t", True
        )
        hash_framework.attacks.collision.connected.loose.database.add(
            algo, db, target, "r", True
        )
    ...
    def add(algo, db, col, prefix="wq", commit=False):
        ...
        et = hash_framework.attacks.collision.build_col_row(
            algo, db, h1s, h1b, h2s, h2b, ""
        )
        ...
        if i == step[0]:
            differential.append(
                models.vars.differential(
                    step[2], "h1i", i * algo.int_size, "h2i", i * algo.int_size
                )
            )
        else:
            differential.append(
                models.vars.differential(
                    start[k], "h1i", i * algo.int_size, "h2i", i * algo.int_size
                )
            )

    hash_framework.models.vars.write_clause(
...
def any_unit_step(algo, start, target):
    ...
    hash_framework.models.vars.write_clause(
        "cdifferentials", tuple(all_differentials), "07-differential.txt"
    )
...
def new_unit_step(algo, start, cols, target):
    ...
    for col in cols:
        ...
        for i in range(0, algo.rounds):
            ...
            differential.append(
                models.vars.differential(
                    col[k], "h1i", i * algo.int_size, "h2i", i * algo.int_size
                )
            )
        negated.append(tuple(differential))
    raw_target = ("and", tuple(all_differentials), ("not", tuple(negated)))
    hash_framework.models.vars.write_clause(
        "cdifferentials", tuple(raw_target), "07-differential.txt"
    )


def distributed_new_neighbor(
    algo,
    base,
    existing,
    poses,
    name="07-differential.txt",
    prefixes=["h1", "h2"],
    out_name="cdifferentials",
):
    ...
    for i in range(0, algo.rounds):
        ...
        if i in poses:
            ...
            for e in existing[i]:
                dlist.append(
                    [e, h1 + "i", i * algo.int_size, h2 + "i", i * algo.int_size]
                )
            dlist.append(
                [base[i], h1 + "i", i * algo.int_size, h2 + "i", i * algo.int_size]
            )
            differential.append(
                ("not", hash_framework.models.vars.choice_differentials(dlist))
            )
        else:
            differential.append(
                models.vars.differential(
                    base[i], "h1i", i * algo.int_size, "h2i", i * algo.int_size
                )
            )
...
def constraints_new_neighbor(algo, cols, pos):
    ...
    for i in range(0, algo.rounds):
        ...
        if i == pos:
            ...
            differential.append(
                models.vars.differential(
                    cols[0][k], "h1i", i * algo.int_size, "h2i", i * algo.int_size
                )
            )
...
def exists_unit_step(algo, start, target, tag="md4-wangs-sasakis-connected"):
    ...
    for e in pos:
        ...
        hash_framework.models.vars.write_values(
            algo.default_state_bits, "h1s", "01-h1-state.txt"
        )
        hash_framework.models.vars.write_values(
            algo.default_state_bits, "h2s", "01-h2-state.txt"
        )
        hash_framework.models.vars.write_assign(
            ["ccollision", "cblocks", "cdifferentials"]
        )
        m.collapse()
        m.build()
        sat = m.run(count=1)
...
def all_neighbors(algo, db, start, tag):
    cols = [start.copy()]
    hash_framework.attacks.collision.insert_db_multiple(
        algo, db, cols, tag + "-original"
    )
    for i in range(0, 48):
        j = 0
        while True:
            ...
            hash_framework.attacks.collision.connected.loose.constraints_new_neighbor(
                algo, cols, i
            )
            hash_framework.attacks.collision.write_constraints(algo)
            hash_framework.attacks.collision.write_optional_differential(algo)
            ...
            for j in range(0, len(jq)):
                ...
                if jqj[0] == fj_job[0]:
                    ...
                    if fj_status:
                        ...
                        rs = m.results(algo)
                        ncols = hash_framework.attacks.collision.build_col_rows(
                            algo, db, rs, tag
                        )
                        cols.extend(ncols)
                        wq.append(jqr)
...
def parallel_find_path(algo, db, start, target, tag):
    cols = [start.copy()]
    min_distance = hash_framework.attacks.collision.metric.loose.distance(
        algo, start, target
    )
    ...
    while len(working_cols) > 0 and min_distance > 0:
        ...
        m.collapse()
        m.build()
        jqj = compute.perform_sat(
            "problem.cnf", "problem.out", count=1, no_wait=True, ident=(wc, i)
        )
        jq.append((wc, i, jqj))
            ...
            for j in range(0, len(jq)):
                ...
                if fj_ident[0] == jqwc and fj_ident[1] == jqi:
                    ...
                    if fj_status:
                        ...
                        ndist = hash_framework.attacks.collision.metric.loose.distance(
                            algo, ncol, target
                        )
                        ...
                        dte = list(
                            map(
                                lambda x: hash_framework.attacks.collision.metric.loose.distance(
                                    algo, ncol, x
                                ),
                                cols,
                            )
                        ...
                        for i in range(0, len(dte)):
                            if dte[i] == 1:
                                ...
                                dr = hash_framework.attacks.collision.metric.loose.delta(
                                    algo, cols[i], cols[nci]
                                )
                                if dr[0][0] not in wqs[i]:
                                    wqs[i].append(dr[0][0])

                                if (
                                    i not in working_cols
                                    and wdist[i][0] <= min_distance
                                ):
                        ...
                        while len(working_cols) == 0 and offset < 4:
                            for i in range(0, len(cols)):
                                if (
                                    hash_framework.attacks.collision.metric.loose.distance(
                                        algo, target, cols[i]
                                    )
                                    <= (min_distance + offset)
                                    and len(wqs[i]) > 0
...
def parallel_find_path_hybrid(algo, db, start, target, tag):
    ...
    while (len(working_cols) > 0 or len(unit_cols) > 0) and min_distance > 1:
        ...
        if len(unit_cols) > 0:
            ...
            hash_framework.models.vars.write_header()
            hash_framework.models.generate(algo, ["h1", "h2"])
            hash_framework.attacks.collision.connected.loose.new_unit_step(
                algo, wcols[wc][0], wcols[wc], target
            )
            hash_framework.attacks.collision.write_constraints(algo)
            hash_framework.attacks.collision.write_optional_differential(algo)
            hash_framework.models.vars.write_assign(
        ...
        while (
            override
            or compute.assign_work() is None
            or (len(working_cols) == 0 and len(unit_cols) == 0 and len(jq) > 0)
        ):
            override = False
            print("Waiting for work...")
            fj = compute.wait_job_hosts(loop_until_found=True)
