...
def __main__():
    for w in [2]:
        for rounds in range(1, 2):
            ...
            m.remote = False
            m.start(
                "scratch-sha3-b" + str(25 * w) + "-r" + str(rounds) + "-collision",
                False,
            )
            n_inputs = set()
            ...
            f = open(
                "/home/cipherboy/GitHub/hash_framework/scratch/collisions-w"
                + str(w)
                + "-r"
                + str(rounds)
                + ".txt",
            ...
            for rs in m.results_generator(algo):
                ...
                for r in range(0, rounds):
                    ...
                    obj["dr" + str(r)] = hf.models.vars.compute_ddelta(
                        rs["h1r" + str(r) + "i"], rs["h2r" + str(r) + "i"]
                    )
                    obj["rr" + str(r)] = hf.models.vars.compute_ddelta(
                        rs["h1r" + str(r) + "i"], rs["h2r" + str(r) + "i"]
                    )
                f.write(str(obj) + "\n")
            f.flush()
